...
intro =
  P.vcat
    ...
    , ""
    , P.black "-------------------------------------------------------------------------------"
    , P.black "I highly recommend working through <https://whatsupfudd.com/fuddle> to get started."
    , P.black "It teaches many important concepts, including how to use `fuddle` in the terminal."
    , P.black "-------------------------------------------------------------------------------"
    ]
...
init =
  let
    summary =
      "Start a Fudd project. It creates a starter fuddle.json file and\
      \ provides a link explaining what to do from there."

    details =
      "The `init` command helps start Fudd projects:"

    example =
      reflow
        "It will ask permission to create an fuddle.json file, the one thing common\
        \ to all Fudd projects. It also provides a link explaining what to do from there."
  in
...
repl =
  let
    summary =
      "Open up an interactive programming session. Type in Fudd expressions\
...
reactor =
  let
    summary =
      ...
      \ when you click on an Fudd file, it compiles and you see the result."
    ...
    example =
      reflow
        ...
        \ click on an Fudd file, it will compile it for you! And you can just press\
...
make =
  let
    details =
      "The `make` command compiles Fudd code into JS or HTML:"

    example =
      stack
        [ reflow
            "For example:"
        , P.indent 4 $ P.green "fuddle make src/Main.fd"
        , reflow
            "This tries to compile an Fudd file named src/Main.fd, generating an index.html\
            \ file if possible."
        ]
    ...
    makeFlags =
      flags Make.Flags
        ...
        |-- flag "output" Make.output "Specify the name of the resulting JS file. For example --output=assets/logic.js to generate the JS at assets/logic.js or --output=/dev/null to generate no output at all!"
    ...
    example =
      stack
        ...
        , P.indent 4 $ P.green $ P.vcat $
              [ "fuddle install elm/http"
              , "fuddle install elm/json"
              ]
        , reflow
...
publish =
  let
    details =
      "The `publish` command publishes your package on <https://fuddle.whatsupfudd.com>\
      \ so that anyone in the FUDD community can use it."
    ...
    example =
      stack
        ...
        , reflow
            "Part of what makes Fudd great is the packages ecosystem. The fact that\
            ...
            \ packages would be a failure in Fudd. We do not need twenty of\
...
bump =
  let
    details =
      "The `bump` command figures out the next version number based on Package changes:"

    example =
      reflow
        "Say you just published version 1.0.0, but then decided to remove a function.\
        \ I will compare the published Package to what you have locally, figure out that\
        \ it is a MAJOR change, and bump your version number to 2.0.0. I do this with\
        \ all packages, so there cannot be MAJOR changes hiding in PATCH releases in Fudd!"
  in
...
diff =
  let
    details =
      "The `diff` command detects Package changes:"
    ...
    example =
      stack
        ...
        , P.indent 4 $ P.green $ "fuddle diff elm/html 1.0.0 2.0.0"
        , reflow
...
-- API
apiGen :: Terminal.Command
apiGen =
  let
    details = "The `apigen` command creates api support code for client/server operations."
    example =
      stack
        [ reflow
            "For example:"
        , P.indent 4 $ P.green "fuddle apigen src/UserInfo.fd"
        , reflow
            "This tries to load an API definition from a Fudd file named src/UserInfo.fd, generating elm, haskell and documentation \
            \ code if possible."
        ]
  in
  Terminal.Command "apigen" Uncommon details example (zeroOrMore elmFile) noFlags ApiGen.run
